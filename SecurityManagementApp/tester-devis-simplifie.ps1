#!/usr/bin/env pwsh
# Script de test pour la création de devis simplifié

# Configuration
$baseUrl = "http://localhost:8080/api/devis"
$contentType = "application/json"

Write-Host "`n===== Test de création d'un devis simplifié =====" -ForegroundColor Cyan

# Création d'un devis sans mission
Write-Host "`nCréation d'un devis sans mission:" -ForegroundColor Green
$devisSansMission = @{
  referenceDevis = "DEV-SIMPLIFIE-$((Get-Date).ToString('yyyyMMdd-HHmmss'))"
  description = "Devis de test simplifié via API"
  statut = "EN_ATTENTE"
  entrepriseId = 1
  clientId = 1
  dateValidite = (Get-Date).AddMonths(1).ToString('yyyy-MM-dd')
  conditionsGenerales = "Conditions générales de test pour devis simplifié"
  missions = @()
}
