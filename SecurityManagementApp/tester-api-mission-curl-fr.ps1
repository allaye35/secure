...
$curlGetAllCommand = "curl -X GET '$baseUrl' -s"
...
"@
    ...
    # Exécution de la commande curl pour mettre à jour la mission
    $curlUpdateCommand = "curl -X PATCH '$baseUrl/$missionId' -H 'Content-Type: application/json' -d '@$tempUpdateFile' -s"
...
$curlSearchCommand = "curl -X GET '$baseUrl/apres?date=$searchDate' -s"
...
$curlSimulateCommand = "curl -X POST '$baseUrl/simuler-calcul' -H 'Content-Type: application/json' -d '@$tempSimulateFile' -s"
...
if ($missionId -ne -1) {
    ...
    $curlDeleteCommand = "curl -X DELETE '$baseUrl/$missionId' -s -v"
    ...
    $curlCheckDeleteCommand = "curl -X GET '$baseUrl/$missionId' -s"
