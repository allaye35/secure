#!/usr/bin/env pwsh
# Script pour tester l'API des missions via PowerShell
...
$missionJson = @{
  titre = "Mission de test via PowerShell"
  description = "Test complet de l'API des missions avec PowerShell"
...
Write-Host ($missionJson | ConvertTo-Json)
...
try {
    ...
    $createResult = Invoke-RestMethod -Method Post -Uri $baseUrl -Headers $headers -Body ($missionJson | ConvertTo-Json) -ErrorAction Stop
...
if ($missionId -ne -1) {
    ...
    Write-Host ($updateJson | ConvertTo-Json)
    
    try {
        $headers = @{
            "Content-Type" = "application/json"
        }
        $updateResult = Invoke-RestMethod -Method Patch -Uri "$baseUrl/$missionId" -Headers $headers -Body ($updateJson | ConvertTo-Json) -ErrorAction Stop
...
Write-Host ($simulateJson | ConvertTo-Json)

try {
    $headers = @{
        "Content-Type" = "application/json"
    }
    $simulateResult = Invoke-RestMethod -Method Post -Uri "$baseUrl/simuler-calcul" -Headers $headers -Body ($simulateJson | ConvertTo-Json) -ErrorAction Stop
