#!/usr/bin/env pwsh
# Script de test pour la création de devis avec mission

# Configuration
$baseUrl = "http://localhost:8080/api/devis"
$contentType = "application/json"

Write-Host "`n===== Test de création d'un devis avec mission =====" -ForegroundColor Cyan
...
$devisAvecMission = @{
  ...
  missions = @(
    @{
      ...
      tarifMissionId = 1
      # Nous omettons le champ statutMission pour voir si le serveur utilise une valeur par défaut
    }
  )
}
